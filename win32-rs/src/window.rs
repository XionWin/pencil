...
use crate::{
    create_window, get_last_error, get_module_handle, register_class, BeginPaint, CreateSolidBrush, DefWindowProcW, DestroyWindow, DispatchMessageW, EndPaint, FillRect, GetMessageW, HBrush, HIcon, HWnd, LPCWStr, LParam, LResult, LoadCursorW, Msg, PanitStruct, PostQuitMessage, Rect, ShowWindow, TranslateMessage, Uint, WParam, WndClassW, CW_USEDEFAULT, IDC_ARROW, SW_SHOW, WM_CLOSE, WM_CREATE, WM_DESTROY, WM_PAINT, WS_CAPTION, WS_OVERLAPPED, WS_SYSMENU
};
...
impl Window {
    pub fn new(width: c_int, height: c_int, title: &str) -> Self {
        ...
        let window_class = WndClassW {
            ...
            h_instance,
            h_icon: 0 as HIcon,
            h_cursor: unsafe { LoadCursorW(std::ptr::null(), IDC_ARROW) } as _,
    ...
    pub fn show(&self) {
        ...
        loop {
            let message_return = unsafe { GetMessageW(&mut msg, std::ptr::null(), 0, 0) };
            if message_return == 0 {
                break;
